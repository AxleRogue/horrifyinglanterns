    ...
    public static void onClientTick(TickEvent.ClientTickEvent event) {
        if (event.phase == TickEvent.Phase.END) {
            ...
            while (KeyHandler.LANTERN_BURST.consumeClick()) {
                ModPackets.INSTANCE.sendToServer(new LanternAbilityPacket(LanternAbilityPacket.AbilityType.BURST));
            }
            while (KeyHandler.LANTERN_LEECH.consumeClick()) {
                ModPackets.INSTANCE.sendToServer(new LanternAbilityPacket(LanternAbilityPacket.AbilityType.LEECH));
            }
        }
    }
}
